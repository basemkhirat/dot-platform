...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        <h2><i class="fa fa-tasks faa-tada animated faa-slow"></i> {!!trans('navs::navs.navigations')!!}</h2>
        <ol class="breadcrumb" style="background: none;">
            <li><a  href="{!!URL::to('/'.ADMIN.'/dashboard')!!}">{!!Lang::get('admin::common.dashboard')!!}</a></li>
            <li><a  href="{!!URL::to('/'.ADMIN.'/navigations')!!}">{!!Lang::get('navs::navs.navigations')!!}</a></li>
            <li>{!!trans('navs::navs.edit')!!} {!!$nav->name!!}</li>
        </ol>
    </div>
    <div class="col-lg-2">
        <a class="btn btn-primary btn-labeled btn-main pull-right" href="{!!route(ADMIN . '.navigations.create')!!}"> <span class="btn-label icon fa fa-plus"></span> {!!Lang::get('navs::navs.add_new')!!}</a> 
    </div>
</div>
@stop
@section("content")
    ...
    <div class="row">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-title">
                <h5> {!!trans('navs::navs.menu_data')!!} </h5>
            </div>
            <div class="ibox-content">
                {!!Form::open(array(
                    'route' => array(ADMIN . '.navigations.update',$nav->id),
                    'method' => 'PUT',
                    'class' => 'form-horizontal',
                    'id' => 'edit-form'
        ))!!}
                <div class="form-group {!!$errors->has('name') ? 'has-error has-feedback' : ''!!}">
                    <label for="menu-name" class="col-sm-2 control-label">{!!trans('navs::navs.menu_name')!!}</label>
                    <div class="col-sm-5">
                        <input name="menu_name" type="text" class="form-control" id="menu-name" placeholder="{!!trans('navs::navs.menu_name')!!}" value="{!!$nav->name!!}">
                        @if($errors->has('name'))<span class="fa fa-times-circle form-control-feedback"></span>@endif
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-9">
                        <button type="submit" class="btn btn-primary">{!!trans('navs::navs.save')!!}</button>
                    </div>
                </div>        </div>
        </div>
                ...
                <div class="row">
                    <div class="row col-sm-6 dd">
                        <div class="panel-heading">
                            <span class="panel-title">{!!trans('navs::navs.items')!!}</span>
                            <div class="pull-right">
                                <button id="save-order" class="btn btn-primary">{!!trans('navs::navs.save_order')!!}</button>
                            </div>
                        </div>
                        <ol id="menu-items" class="dd-list" data-navigation-id='{!!$nav->id!!}' style="margin-top:10px">
                            @include('navs::partials.items')
                        </ol>
                    </div>
                                        ...
                                        <div class="panel-body">
                                            <form class="search-form">
                                                <div class="input-group" style="margin-bottom: 7px;">
                                                    <input type="text" class="form-control search-text" placeholder="{!!trans('navs::navs.search')!!}" id="posts-search-text">
                                                    <span class="input-group-btn">
                                                        <button class="btn search-go" type="button" data-text-source="posts-search-text" data-for="posts">{!!trans('navs::navs.filter')!!}</button>
                                                    </span>
                                                </div>
                                            </form>
                                ...
                                <div class="panel panel-primary">
                                    ...
                                    <div id="collapseFour-success" class="panel-collapse collapse" style="height: 0px;">
                                        <div class="panel-body">
                                            <form class="search-form">
                                                <div class="input-group" style="margin-bottom: 7px;">
                                                    <input type="text" class="form-control" placeholder="{!!trans('navs::navs.search')!!}" id="pages-search-text">
                                                    <span class="input-group-btn">
                                                        <button class="btn search-go" type="button" data-text-source="pages-search-text" data-for="pages">{!!trans('navs::navs.filter')!!}</button>
                                                    </span>
                                                </div>
                                            </form>
                                ...
                                <div class="panel panel-primary">
                                    ...
                                    <div id="collapseTwo-success" class="panel-collapse collapse" style="height: 0px;">
                                        <div class="panel-body">
                                            <form class="search-form">
                                                <div class="input-group" style="margin-bottom: 7px;">
                                                    <input type="text" class="form-control" placeholder="{!!trans('navs::navs.search')!!}" id="category-search-text">
                                                    <span class="input-group-btn">
                                                        <button class="btn search-go" type="button" data-text-source="category-search-text" data-for="categories">{!!trans('navs::navs.filter')!!}</button>
                                                    </span>
                                                </div>
                                            </form>
                                        ...
                                        <div class="panel-body">
                                            <form action="#" class="form-horizontal">
                                                ...
                                                <div class="row form-group">
                                                    <div class="col-sm-8">
                                                        <input type="text" id="url-text" name="text" placeholder="{!!trans('navs::navs.text')!!}" class="form-control form-group-margin">
                                                    </div>
                                                </div>
                                                <div class="row form-group">
                                                    <div class="col-sm-8">
                                                        <input type="URL" id="url-url" name="url" placeholder="{!!trans('navs::navs.url')!!}" class="form-control form-group-margin">
                                                    </div>
                                                </div>
                                                <div class="panel-footer text-right">
                                                    <button class="btn btn-primary" id="url-add">{!!trans('navs::navs.add')!!}</button>
                                                </div>
                                            </form>
                                        </div> <!-- / .panel-body -->
                                    </div> <!-- / .collapse -->
...
$(document).ready(function () {
    ...
    jQuery.extend(jQuery.validator.messages, {
        required: "{!!Lang::get('navs::groups.required')!!}",
    });
