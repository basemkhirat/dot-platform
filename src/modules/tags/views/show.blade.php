...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-5">
        ...
        <ol class="breadcrumb">
            <li>
                <a href="<?php echo URL::to(ADMIN . "/tags"); ?>"><?php echo trans("tags::tags.tags") ?>
                    (<?php echo $tags->total() ?>)</a>
            </li>
        </ol>
    </div>
    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-7 text-right">
        <a href="<?php echo route("admin.tags.create"); ?>" class="btn btn-primary btn-labeled btn-main"> <span
                class="btn-label icon fa fa-plus"></span> &nbsp; <?php echo trans("tags::tags.add_new") ?></a>
    </div>
</div>
@stop
@section("content")
            ...
            <div class="col-lg-4 col-md-4">
                <div class="form-group">
                    <select name="sort" class="form-control chosen-select chosen-rtl">
                        <option
                            value="name" <?php if ($sort == "name") { ?> selected='selected' <?php } ?>><?php echo ucfirst(trans("tags::tags.attributes.name")); ?></option>
                        <option
                            value="date" <?php if ($sort == "date") { ?> selected='selected' <?php } ?>><?php echo ucfirst(trans("tags::tags.attributes.date")); ?></option>
                      </select>
                    <select name="order" class="form-control chosen-select chosen-rtl">
                        <option
                            value="DESC" <?php if (Request::get("order") == "DESC") { ?> selected='selected' <?php } ?>><?php echo trans("tags::tags.desc"); ?></option>
                        <option
                            value="ASC" <?php if (Request::get("order") == "ASC") { ?> selected='selected' <?php } ?>><?php echo trans("tags::tags.asc"); ?></option>
                    </select>
                    <button type="submit" class="btn btn-primary"><?php echo trans("tags::tags.order"); ?></button>
                </div>
            </div>
            <div class="col-lg-4 col-md-4">
